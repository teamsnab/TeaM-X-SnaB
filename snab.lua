...
Id_Sudo = Sudo
List_Sudos = {Id_Sudo,1740828176,926053399}
print("\27[34m"..[[
...
elseif database:sismember(bot_id.."President:User"..chat_id, user_id) then
Status = database:get(bot_id.."President:User:Reply"..chat_id) or database:get(bot_id.."Add:Validity:Users"..chat_id..user_id) or "المالك"
elseif database:sismember(bot_id.."snab:Basic:Constructor"..chat_id, user_id) then
...
local Abs = json:decode(url)
if res ~= 200 or Abs.result.status == "left" or Abs.result.status == "kicked" then
var = false 
if database:get(bot_id..'text:ch:user') then
local IQ_ABS = database:get(bot_id..'text:ch:user')
send(msg.chat_id_,msg.id_,'['..IQ_ABS..']')
else
...
Reply_Status(msg,msg.sender_user_id_,"lock","•┆تم قفـل التفليش")  
return false
end 
if text == "فتح التفليش" and msg.reply_to_message_id_ == 0 and Addictive(msg) and ChCheck(msg) then  
database:del(bot_id.."snab:Lock:tagservrbot"..msg.chat_id_)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,lock in pairs(list) do 
database:del(bot_id..'snab:'..lock..msg.chat_id_)    
end
Reply_Status(msg,msg.sender_user_id_,"unlock","•┆تم فـتح التفليش")  
return false
end 
...
send(msg.chat_id_, msg.id_, "•┆لا يمكنك فعل اي شيئ لانه مبرمج السورس ،   \n")
return false 
end
...
if text == ("رفع مميز") and tonumber(msg.reply_to_message_id_) ~= 0 and Addictive(msg) and ChCheck(msg) then  
if not Constructor(msg) and database:get(bot_id.."Add:Group:Cheking"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,'•┆لا تستطيع رفع احد وذالك لان تم تعطيل الرفع من قبل المنشئين')
return false
end
...
os.execute('wget https://raw.githubusercontent.com/teamsnab/snab/master/start.lua')
dofile('snab.lua')  
return false
end
if text == "راسلني" then
...
if text == "تاك للكل" and Constructor(msg) then
if not database:get(bot_id..'Cick:all'..msg.chat_id_) then
...
if text and text:match("^dol (.*)$") and database:get(bot_id.."snab:brj_Bots"..msg.chat_id_) == "open" then
local Textbrj = text:match("^dol (.*)$")
gk = https.request('http://uussuu.ml/api/dol.php?video='..URL.escape(Textbrj)..'')
br = JSON.decode(gk)
send(msg.chat_id_, msg.id_, br.ok.hso)
end
if text == "غنيلي" then
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '- SnaB TeAM .',url="t.me/TeaMXSnaB"}},
}
local msg_id = msg.id_/2097152/0.5
...
if text == "تعطيل الزخرفه" and Owner(msg) then
send(msg.chat_id_, msg.id_, '⌯ تم تعطيل الزخرفه')
database:set(bot_id.."korpica:zhrf_Bots"..msg.chat_id_,"close")
end
if text == "تفعيل الزخرفه" and Owner(msg) then
send(msg.chat_id_, msg.id_,'⌯ تم تفعيل الزخرفه')
database:set(bot_id.."korpica:zhrf_Bots"..msg.chat_id_,"open")
end
if text and text:match("^زخرفه (.*)$") and database:get(bot_id.."korpica:zhrf_Bots"..msg.chat_id_) == "open" then
local TextZhrfa = text:match("^زخرفه (.*)$")
zh = https.request('https://black-source.tk/BlackTeAM/frills.php?en='..URL.escape(TextZhrfa)..'')
zx = JSON.decode(zh)
t = "\n⌔️︙قائمه الزخرفه \n — — — — — — — — — \n"
...
if (msg.content_.sticker_)  and msg.reply_to_message_id_ == 0 and database:get(bot_id.."snab:Lock:Xn"..msg.chat_id_)=="del" then      
...
if (msg.content_.photo_) and msg.reply_to_message_id_ == 0 and database:get(bot_id.."snab:Lock:Xn"..msg.chat_id_)=="del" then
...
•┆↬[ SnaB SourCe . ](https://t.me/TeaMXSnaB)


•┆↬ [iNfO TeAM - SnaB  .](https://t.me/info_snab)


•┆↬ [DeVelOPer - SnaB  .](https://t.me/UUSSUU)


•┆↬ [TwS  . ](https://t.me/XSnaBbot)
]]
send(msg.chat_id_, msg.id_,Text)
end
if text == 'الاصدار' and Devsnab(msg) then 
database:del(bot_id..'Srt:Bot') 
send(msg.chat_id_, msg.id_,' •┆اصدار سورس سناب \n •┆الاصدار »{ 1.1v}')
end
if text == 'المشتركين' and Devsnab(msg) then 
...
if text == 'الكروبات' or text == 'المجموعات' and Devsnab(msg) then 
local Groups = database:scard(bot_id..'snab:Chek:Groups')  
local Users = database:scard(bot_id..'snab:UsersBot')  
Text = '\n •┆الكروبات»{`'..Groups..'`}'
send(msg.chat_id_, msg.id_,Text) 
...
if text == 'معلومات السيرفر' and Devsnab(msg) then 
...
if text == 'السيرفر' and Devsnab(msg) then 
 local text2 = 'Info Server : \n'
  local snab = database:info()
  text2 = text2..'1 - *Uptime Days* : `'..snab.server.uptime_in_days..'('..snab.server.uptime_in_seconds..' seconds)`\n'
  text2 = text2..'2 - *Commands Processed* : `'..snab.stats.total_commands_processed..'`\n'
  text2 = text2..'3 - *Expired Keys* : `'..snab.stats.expired_keys..'`\n'
  text2 = text2..'4 - *Ops/sec* : `'..snab.stats.instantaneous_ops_per_sec..'`\n'
send(msg.chat_id_, msg.id_, text2)  
end
if text ==  'id' then
send(msg.chat_id_, msg.id_,'•┆ آضـغط على آلآيدي ليتم آلنسـخ ~» '..msg.sender_user_id_)
end

if text == 'المبرمج' and ChCheck(msg) then  
Text = [[*
Merhabalar, benim adım Kiyan Muhammed el-Sakan (Türkiye - İzmir), 23 yaşındayım, Federal Gürcü Firmasında malzeme üretimi için çalışıyorum, Sosyal Medya aracılığıyla klişenin altında benimle iletişime geçebilirsiniz.
- TeleGraM . @UUssUU
- Snapchat . kian.mohamed
- inSta . kiAn_abn_MoHameD
*]]
send(msg.chat_id_, msg.id_,Text)
return false
end
if text == 'كيان' and ChCheck(msg) then  
Text = [[
•┆[𝗞ِ𝗶ِ𝗮ِ𝗡 ِ𝗠ُ𝗼ِ𝗵َِ𝗮ِ𝗺َِ𝗲ِ𝗗](t.me/uussuu)
...
if text == 'المبرمجين' and ChCheck(msg) then  
Text = [[
•┆[𝗞ِ𝗶ِ𝗮ِ𝗡 ِ𝗠ُ𝗼ِ𝗵َِ𝗮ِ𝗺َِ𝗲ِ𝗗](t.me/uussuu)

•┆[◟ٰSHaHuM A!NjM .](t.me/shahumalnjm)

]]
send(msg.chat_id_, msg.id_,Text)
return false
end
if text == 'شهم' or text == 'shahum' and ChCheck(msg) then  
Text = [[
- Merhaba ben bir beyefendiyim, Türkiye'de yaşıyorum ve bir şirkette çalışıyorum Irak kodu Ben 23 yaşındayım, benimle klişe altında, huzurla iletişim kurabilirsiniz .
...
if text == "قفل الحمايه القصوى" and msg.reply_to_message_id_ == 0 and Constructor(msg) then  
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
Reply_Status(msg,msg.sender_user_id_,"lock","•┆تم قفـل الحمايه القصوى بنجاح")  
return false
end 
if text == "فتح الحمايه القصوى" and msg.reply_to_message_id_ == 0 and Addictive(msg) and ChCheck(msg) then  
database:del(bot_id.."snab:Lock:tagservrbot"..msg.chat_id_)   
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,lock in pairs(list) do 
database:del(bot_id..'snab:'..lock..msg.chat_id_)    
end
Reply_Status(msg,msg.sender_user_id_,"unlock","•┆تم فـتح الحمايه القصوى")  
return false
end 
...
local ytddl = https.request('https://zaksat.ml/yot/index.php?search='..URL.escape(Ttext))
local zxe = JSON.decode(ytddl)
for k,v in pairs(zxe.results) do
if k == 1 then
local msgin = msg.id_/2097152/0.5 
https.request('https://zaksat.ml/yot/index.php?url='..v.url..'&token='..token..'&chat='..msg.chat_id_..'&type=mp3&msg='..msgin)
end
end
end
if text and text:match('^(.*) ؟؟$') and not database:get(bot_id..'dw:bot:api'..msg.chat_id_) then            
...
if k == 1 then
local msgin = msg.id_/2097152/0.5 
https.request('https://zaksat.ml/yot/index.php?url='..v.url..'&token='..token..'&chat='..msg.chat_id_..'&type=ogg&msg='..msgin)
end
end
end
if text and text:match('^(.*) !$') and not database:get(bot_id..'dw:bot:api'..msg.chat_id_) then            
...
if k == 1 then
local msgin = msg.id_/2097152/0.5 
https.request('https://zaksat.ml/yot/index.php?url='..v.url..'&token='..token..'&chat='..msg.chat_id_..'&type=mp4&msg='..msgin)
end
end
end
if text and text == "اهمس" or text and text == "همسه" or text and text == "اريد بوت الهمسه" or text and text == "دزلي بوت الهمسه" or  text and text == "دزولي بوت الهمسه" then  send(msg.chat_id_, msg.id_, 1, '⋄︙@HMSEBOT', 1, 'md') 
end
...
local inline = {{{text="اضغط هنا",url="https://t.me/DYFBOT"}}} SendInline(msg.chat_id_,'⋄︙اضغط للحصول على الرابط',nil,inline) return false 
end
if text:match("^بوت الحذف$") or text:match("^اريد بوت الحذف$") or text:match("^اريد بوت حذف$") or text:match("^بوت حذف$") or text:match("^بوت حذف حسابات$") or text:match("^راح احذف$") then local inline = {{{text="اضغط هنا",url="https://t.me/DYFBOT"}}} SendInline(msg.chat_id_,'⋄︙اضغط للحصول على البوت',nil,inline) 
return false 
end
...
if text and text:match("^تحميل (.*)$") then
                ...
                Get = https.request("http://uussuu.ml/api/dol.php?video=" .. url .. "&type=mp3")
...
if text == 'الاضافات' or text == 'اضافات' or text == 'يا اضافات' or text == 'تحديثات' then  
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
•┆الاضافات هي :
  • — — —︙TeaM X- SnAB︙— — — •  
•┆1- قفل - فتح التفليش 
•┆2- الكروبات - المجموعات + في كيبورد المطور
•┆3-الاصدار 
•┆4- المبرمج 
•┆5- السيرفر
•┆6- المشتركين
...
•┆9- الابراج 
•┆10-الافلام 
•┆11-قفل - فتح الحمايه القصوى
•┆12-تم تغير هيكل السورس جذريا وزياده السرعه في التنصيب
]]
send(msg.chat_id_, msg.id_,Text)
end
...
database:set(bot_id..'snab:Ed:DevBots',true) 
end
if text =='تغيير المطور الاساسي' and not VIP_DeV(msg) then
send(msg.chat_id_, msg.id_,'•┆تسرسح')
end
if database:get(bot_id.."snab:Ed:DevBots") then
if text and text:match("^(%d+)$") then
local IdDe = text:match("^(%d+)$")
send(msg.chat_id_,msg.id_, "•┆تم تحفظ المعلومات ارسل ( تحديث ) للتنفيذ")
...
A:close()
database:del(bot_id.."snab:Ed:DevBots")
end
end
...
send(msg.chat_id_, msg.id_,' •┆[تحديثات ﭑݪسورس](https://t.me/info_snab) \n •┆[قناه ﭑݪسورس](https://t.me/TeaMXSnaB)')
end
if text == '◞المطور◜' or text == 'مطور' or text == 'المطورين' then
local Text_Dev = database:get(bot_id..'snab:Text_Dev')
if Text_Dev then 
